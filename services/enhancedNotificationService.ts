...
export class EnhancedNotificationService {
  ...
  static async scheduleAnalyticsNotifications(
    ...
    try {
      ...
      const shouldSend = await this.shouldSendAnalyticsNotification(
        userId,
        preferences
      );
      ...
      if (consumptionAnalytics && preferences.budgetAlerts) {
        notifications.push(
          ...this.generateConsumptionNotifications(consumptionAnalytics)
        );
      }
  ...
  static async scheduleAchievementNotifications(
    ...
    try {
      ...
      if (!preferences.achievementsEnabled || newAchievements.length === 0)
        return;
  ...
  private static generateWasteNotifications(
    ...
    if (analytics.wasteValue > 30) {
      notifications.push({
        ...
        body: `You've wasted $${analytics.wasteValue.toFixed(
          0
        )} worth of food recently. Check your waste report for tips to improve!`,
        data: {
          type: "waste_alert",
          screen: "waste-report",
        },
    ...
    if (analytics.wasteReductionProgress > 20) {
      notifications.push({
        ...
        body: `You've reduced food waste by ${analytics.wasteReductionProgress.toFixed(
          0
  ...
  private static generateConsumptionNotifications(
    ...
    if (analytics.budgetAnalysis.variance > 20) {
      notifications.push({
        ...
        title: "💳 Budget Alert",
        body: `You're $${analytics.budgetAnalysis.variance.toFixed(
          0
        )} over budget this period. Review your consumption report for savings tips.`,
        data: {
          type: "budget_alert",
    ...
    if (analytics.budgetAnalysis.variance < -10) {
      notifications.push({
        ...
        body: `You're $${Math.abs(analytics.budgetAnalysis.variance).toFixed(
          0
        )} under budget! Excellent financial management.`,
        data: {
          type: "celebration",
          screen: "consumption-report",
        },
    ...
    const healthyCategories = analytics.nutritionalInsights.filter((item) =>
      ["fruits", "vegetables"].includes(item.category.toLowerCase())
    );

    if (healthyCategories.length > 0) {
      const avgHealthyScore =
        healthyCategories.reduce(
          (sum, item) => sum + item.consumptionScore,
          0
  ...
  static async scheduleWeeklySummary(userId: string): Promise<void> {
    try {
      ...
      if (!preferences.insightsEnabled || preferences.frequency !== "weekly")
        return;
    ...
    try {
      await Notifications.scheduleNotificationAsync({
        ...
        trigger: ((): Notifications.NotificationTriggerInput => {
          const t = notification.trigger as any;
          if (typeof t.seconds === "number") {
            return {
              type: Notifications.SchedulableTriggerInputTypes.TIME_INTERVAL,
              seconds: t.seconds,
              repeats: Boolean(t.repeats),
            } as any;
          }
          return {
  ...
  private static async updateLastAnalyticsNotification(
    userId: string
  ): Promise<void> {
    try {
      await AsyncStorage.setItem(
  ...
  static async handleAnalyticsUpdate(
    ...
    await this.scheduleAnalyticsNotifications(
      userId,
      wasteAnalytics,
    ...
    if (wasteAnalytics && consumptionAnalytics) {
      const newAchievements =
        await AchievementService.updateAchievementsFromAnalytics(
          userId,
          wasteAnalytics,
          consumptionAnalytics
        );
