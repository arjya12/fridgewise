...
import { supabase, FoodItem, UsageLog } from '@/lib/supabase';
...
export const foodItemsService = {
  ...
  // Add a new food item
  async addItem(item: Omit<FoodItem, 'id' | 'user_id' | 'created_at' | 'updated_at'>) {
  ...
  async getUsageStats(startDate?: Date, endDate?: Date) {
    ...
    if (total > 0) {
      stats.wastePercentage = ((stats.totalExpired + stats.totalWasted) / total) * 100;
    }
