...
export function getCalendarDotColors(items: FoodItem[]): Array<{
  ...
  if (urgencyGroups.critical.length > 0) {
    dots.push({
      key: "critical",
      color: "#EF4444", // Phase 2 critical color
      selectedDotColor: "#FFFFFF",
    });
  } else if (urgencyGroups.warning.length > 0) {
    dots.push({
      key: "warning",
      color: "#F97316", // Phase 2 warning color
      selectedDotColor: "#FFFFFF",
    });
  } else if (urgencyGroups.soon.length > 0) {
    dots.push({
      key: "soon",
      color: "#EAB308", // Phase 2 soon color
      selectedDotColor: "#1F2937", // Dark text for yellow background
    });
  } else if (urgencyGroups.safe.length > 0) {
    dots.push({
      key: "safe",
      color: "#22C55E", // Phase 2 safe color
      selectedDotColor: "#FFFFFF",
    });
  }
